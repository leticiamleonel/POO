...
import com.POO.gestaoDisk.modelo.conexao.Conexao;
import com.POO.gestaoDisk.modelo.conexao.ConexaoMysql;
...
import com.POO.gestaoDisk.modelo.entidades.Categoria;
...
import com.POO.gestaoDisk.modelo.util.ProdutoTableModel;
...
public class ProdutoController implements ActionListener, MouseListener, KeyListener {
    ...
    private void validacaoDaCategoria() {
        if(this.dashboard.getComboBoxProdutoCategoria().getSelectedIndex() == 0) {
            String mensagem = "Deve preencher a categoria";
            mensagemNaTela(mensagem, Color.RED);
            throw new NegocioException(mensagem);
        }
    }
    ...
    private void apagar() {
        ...
        if(autenticacaoDao.temPermissao(usuario)){
            if(this.produto != null) {
                ...
                JOptionPane.showMessageDialog(dashboard, "Deve selecionar um produto na tabela", "Selecionar um produto", 0);
            }
        }
    }
